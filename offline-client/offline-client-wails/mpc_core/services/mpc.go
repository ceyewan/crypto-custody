...
//   - []byte: 加密后的本地分片数据
//   - error: 操作过程中遇到的错误
func (s *MPCService) KeyGeneration(ctx context.Context, managerAddr, room string, threshold, parties, partyIndex int, filename, recordID string) (string, string, []byte, error) {
	clog.Debug("开始密钥生成",
		clog.Int("threshold", threshold),
		clog.Int("parties", parties),
		clog.Int("party_index", partyIndex),
	...
	// 运行密钥生成命令
	clog.Info("执行密钥生成")
	if err := utils.RunKeyGen(ctx, s.cfg, managerAddr, room, threshold, parties, partyIndex, filePath); err != nil {
	...
	// 将随机数存储到安全芯片
	clog.Info("存储密钥到安全芯片")
	if err := s.securityService.StoreData(recordID, address, randomKey); err != nil {
		clog.Error("存储密钥失败", clog.Err(err))
		return "", "", nil, fmt.Errorf("存储密钥到安全芯片失败: %w", err)
	}

	clog.Debug("密钥生成完成", clog.String("addr", address))
	return address, publicKey, encryptedData, nil
}
...
//   - recordID: SE 记录编号
//   - address: 以太坊地址
//   - encryptedShard: 加密后的本地分片数据
//   - signature: 安全芯片读取操作的授权签名
//
// 返回:
//   - string: 签名结果
//   - error: 操作过程中遇到的错误
func (s *MPCService) SignMessage(ctx context.Context, managerAddr, room string, signingIndex int, parties, data, filename, recordID, address string, encryptedShard, signature []byte) (string, error) {
	clog.Debug("开始消息签名",
	...
	// 从安全芯片读取随机数
	clog.Info("从安全芯片读取密钥")
	randomKey, err := s.securityService.ReadData(recordID, address, signature)
	if err != nil {
	...
	// 使用随机数解密数据
	clog.Info("解密密钥数据")
	decryptedData, err := utils.DecryptAES(encryptedShard, randomKey)
	if err != nil {
	...
	// 运行签名命令
	clog.Info("执行MPC签名")
	signResult, err := utils.RunSigning(ctx, s.cfg, managerAddr, room, signingIndex, parties, data, filePath)
	if err != nil {
